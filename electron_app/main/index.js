...
// ========== FIXED: grantAccess now stores the access locally ==========
ipcMain.handle('contract:grantAccess', async (event, { patientDid, doctorDid, documentCid, encryptedCid, expiryTime }) => {
    console.log('📝 contract:grantAccess storing:', { patientDid, doctorDid, documentCid, encryptedCid, expiryTime });
    try {
        // Store in global accesses list
        const accesses = store.get('accessGrants') || [];
        const grant = {
            patientDid,
            doctorDid,
        ...
        // Also store specifically for the doctor for quick retrieval
